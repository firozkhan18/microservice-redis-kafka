...
import com.springboot.microservice.adapters.aop.ExceptionHandlerAdvice;
import com.springboot.microservice.server.config.ServiceConfiguration;
import com.springboot.microservice.server.controllers.HealthController;
...
/**
 * Micro Service - Spring Boot Application API URL :
 * http://localhost:9090/service/api/v1/swagger-ui.html
 *
 * @author Firoz Khan
 */
@EnableScheduling
...
@SpringBootApplication(scanBasePackages = { "com.springboot.microservice" })
//@EnableJpaRepositories(basePackages = "com.springboot.microservice.domain.entities.order") // Adjust the package as needed
...
public class DistributedCachingApplication {
	...
	private final String title = "<h1>Welcome to MICRO service<h1/>" + "<h3>Copyright (c) COMPANY, 2022</h3>"
	...
	public static void start(String[] args) {
		...
		try {
			context = SpringApplication.run(DistributedCachingApplication.class, args);
	...
	/**
	 * Open API v3 Docs - All Reference: https://springdoc.org/faq.html
	 * 
	 * @return
	 */
	@Bean
	public GroupedOpenApi allPublicApi() {
		return GroupedOpenApi.builder().group(serviceConfig.getServiceName() + "-service")
				.pathsToMatch(serviceConfig.getServiceApiPath() + "/**").build();
	}

	/**
	 * Open API v3 Docs - MicroService Reference: https://springdoc.org/faq.html
	 * 
	 * @return
	 */
	@Bean
	public GroupedOpenApi appPublicApi() {
		return GroupedOpenApi.builder().group(serviceConfig.getServiceName() + "-service-core")
	...
	/**
	 * Open API v3 Docs - Core Service Reference: https://springdoc.org/faq.html
	...
	public GroupedOpenApi configPublicApi() {
		// System.out.println;
		return GroupedOpenApi.builder().group(serviceConfig.getServiceName() + "-service-config")
				.pathsToMatch(serviceConfig.getServiceApiPath() + "/config/**").build();
	}

	@Bean
	public GroupedOpenApi systemPublicApi() {
		return GroupedOpenApi.builder().group(serviceConfig.getServiceName() + "-service-health")
				.pathsToMatch(serviceConfig.getServiceApiPath() + "/service/**").build();
	}

	/**
	 * Open API v3 Reference: https://springdoc.org/faq.html
	 * 
	 * @return
	 */
	@Bean
	public OpenAPI buildOpenAPI() {
		return new OpenAPI().servers(getServers())
				.info(new Info().title(serviceConfig.getServiceName() + " Service")
						.description(serviceConfig.getServiceDetails()).version(serviceConfig.getServerVersion())
						.license(new License().name("License: " + serviceConfig.getServiceLicense())
								.url(serviceConfig.getServiceUrl())))
				.externalDocs(
						new ExternalDocumentation().description(serviceConfig.getServiceName() + " Service Source Code")
	...
	/**
	 * All file upload till 512 MB returns MultipartConfigElement
	 * 
	 * @return
	 */
//	@Bean
...
//	}
	...
	public jakarta.servlet.MultipartConfigElement multipartConfigElement() {
		MultipartConfigFactory factory = new MultipartConfigFactory();
		factory.setMaxFileSize(DataSize.ofBytes(500_000_000L)); // 500 MB
		factory.setMaxRequestSize(DataSize.ofBytes(500_000_000L)); // Optional: set max request size
		return factory.createMultipartConfig();
	}
